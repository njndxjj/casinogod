 ...
 <head>
  ...
  <body>
   ...
   %>   
    ...
    <br> user account :  <%=(String)session.getAttribute("username")%>  </br>
    <tr>  
        <th width="39%">编号</th>  
        <th width="39%">游戏类型</th> 
        <th width="39%">消费类型</th>
    ...
    <s:iterator id="itemHistory" value="#request.itemHistorys"> 
    <tr>  
         <td>${itemHistory.id}</td>
         <td>${itemHistory.gameType}</td>  
          <s:if test='%{#itemHistory.purchaseType==0}'>
            <td>购买</td>
           </s:if>
           <s:elseif test='%{#itemHistory.purchaseType==1}'>
            <td>消费</td>
           </s:elseif>
           <s:else>
            <td>赠送</td>
           </s:else>
         <td>${itemHistory.itemName}</td>
         <td>${itemHistory.itemNum}</td>
         <td>${itemHistory.giftUserId}</td>  
         <td>${itemHistory.purchaseTime}</td>
         <td>${itemHistory.otherNotes}</td> 
    </tr>  
 </s:iterator> 
</table>  
          </div>
