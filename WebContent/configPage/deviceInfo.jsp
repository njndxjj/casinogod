...
//	alert(document.getElementById("firstPage").href+"&pageSize="+field.value);
  ...
  <body>
   ...
   %>   
    ...
    <tr>  
        <th width="39%">账号</th> 
        <th width="39%">设备token</th>
        <th width="39%">注册时间</th>
        <th width="39%">说明</th>
        <th width="39%">操作</th>
    </tr> 
    <s:iterator id="device" value="#request.userDevice"> 
    <tr>  
         <td>${device.userId}</td>
         <td>${device.deviceToken}</td>  
         <td>${device.registerTime}</td>
         <td>${device.notes}</td>
         <td><a href="configUpdateDevice.action?userId=${device.userId}&deviceToken=${device.deviceToken}">更新</a>
         <a href="deleteDevice.action?userId=${device.userId}">删除</a>
         </td>
    </tr>  
 </s:iterator> 
</table>  
