...
<title>AccountProfile manager</title>
  ...
  <body>
   ...
   %>   
    ...
    <tr>  
        ...
        <th width="39%">状态</th> 
        <th width="39%">用户类型</th>
        <th width="39%">SNSID</th>
        <th width="39%">SNSToken</th>  
        <th width="39%">操作</th> 
    </tr> 
    <c:forEach items="${pm.datas}" var="userAccount">
    <tr>  
         <td><c:out value="${userAccount.account}"/></td>
         <td><c:out value="${userAccount.password}"/></td>
         <c:choose>
          <c:when test='${userAccount.freeze==0}'>
            <td>可用</td>
         </c:when>
           <c:when test='${userAccount.freeze==1}'>
            <td>冻结</td>
           </c:when> 
           </c:choose>
           <td><c:out value="${userAccount.userType}"/></td>
           <td><c:out value="${userAccount.snsId}"/></td>
           <td><c:out value="${userAccount.snsToken}"/></td>
        <td><a href="configPage/updatePassword.jsp?account=<c:out value="${userAccount.account}"/>">更新</a></td>   
    </tr>  
 </c:forEach> 
</table> 
<pg:pager url="accountInfo.action" items="${pm.total}" export="currentPageNumber=pageNumber" maxPageItems="${pageSize}" maxIndexPages="10">  
     <pg:first>  
        <a id="firstPage" href="${pageUrl}">首页</a>  
    </pg:first>  
    <pg:prev>  
        <a href="${pageUrl}&pageSize=${pageSize}">前一页</a>  
    </pg:prev>  
    <pg:pages>  
        <c:choose>  
            ...
            <c:otherwise>  
                <a href="${pageUrl}&pageSize=${pageSize}">${pageNumber }</a>  
            </c:otherwise>  
        </c:choose>  
    </pg:pages>  
    <pg:next>  
        <a href="${pageUrl }&pageSize=${pageSize}">下一页</a>  
    </pg:next>  
    <pg:last>  
        <a href="${pageUrl}&pageSize=${pageSize}">末页</a>  
    </pg:last>  
    &nbsp;&nbsp;&nbsp;&nbsp;
        设置记录数:
    <select name="pageSize" onchange="selectPagesize(this)" class="span1">
      <c:forEach begin="10" end="50" step="10" var="i">
       <option value="${i}" <c:if test="${i eq pageSize}">selected</c:if> >${i}</option> 
      </c:forEach>
    </select>   
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        共有<strong>${pm.total}</strong> 条记录
